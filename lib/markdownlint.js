...
import { length, concat } from "./rules";
import { isString, isEmptyString, newLineRe, forEachInlineCodeSpan, inlineCommentRe, clearHtmlCommentText, getLineMetadata, flattenLists, isNumber, isObject, utf8Encoding, frontMatterRe } from "../helpers";
import { lineMetadata, flattenedLists, clear } from "./cache";
...
function validateRuleList(ruleList) {
  ...
  ruleList.forEach(function forRule(rule, index) {
    const customIndex = index - length;
    ...
    [ "names", "tags" ].forEach(function forProperty(property) {
      ...
      if (!result &&
        (!value || !Array.isArray(value) || (value.length === 0) ||
         !value.every(isString) || value.some(isEmptyString))) {
...
function removeFrontMatter(content, frontMatter) {
  ...
  if (frontMatter) {
    ...
    if (frontMatterMatch && !frontMatterMatch.index) {
      ...
      frontMatterLines = contentMatched.split(newLineRe);
...
function getEnabledRulesPerLineNumber(
  ...
  function handleInlineConfig(perLine, forEachMatch, forEachLine) {
    ...
    input.forEach((line) => {
      if (!noInlineConfig) {
        let match = null;
        while ((match = inlineCommentRe.exec(line))) {
...
function lintContent(
  ...
  content = clearHtmlCommentText(removeFrontMatterResult.content);
  ...
  lineMetadata(getLineMetadata(params));
  flattenedLists(flattenLists(params));
  ...
  function forRule(rule) {
    ...
    function onError(errorInfo) {
      if (!errorInfo ||
        !isNumber(errorInfo.lineNumber) ||
        (errorInfo.lineNumber < 1) ||
        (errorInfo.lineNumber > lines.length)) {
        throwError("lineNumber");
      }
      if (errorInfo.detail &&
        !isString(errorInfo.detail)) {
        throwError("detail");
      }
      if (errorInfo.context &&
        !isString(errorInfo.context)) {
        throwError("context");
      }
      if (errorInfo.range &&
        (!Array.isArray(errorInfo.range) ||
         (errorInfo.range.length !== 2) ||
         !isNumber(errorInfo.range[0]) ||
         (errorInfo.range[0] < 1) ||
         !isNumber(errorInfo.range[1]) ||
         (errorInfo.range[1] < 1) ||
      ...
      if (fixInfo) {
        ...
        if (fixInfo.lineNumber !== undefined) {
          if ((!isNumber(fixInfo.lineNumber) ||
        ...
        if (fixInfo.editColumn !== undefined) {
          if ((!isNumber(fixInfo.editColumn) ||
        ...
        if (fixInfo.deleteCount !== undefined) {
          if ((!isNumber(fixInfo.deleteCount) ||
        ...
        if (fixInfo.insertText !== undefined) {
          if (!isString(fixInfo.insertText)) {
...
function lintFile(
  ...
  if (synchronous) {
    lintContentWrapper(null, fs.readFileSync(file, utf8Encoding));
  } else {
    fs.readFile(file, utf8Encoding, lintContentWrapper);
  }
}
...
function lintInput(options, synchronous, callback) {
  ...
  const ruleList = concat(options.customRules || []);
  ...
  const frontMatter = (options.frontMatter === undefined) ?
    frontMatterRe : options.frontMatter;
...
function readConfig(file, parsers, callback) {
  ...
  // Read file
  fs.readFile(file, utf8Encoding, (err, content) => {
    if (err) {
      return callback(err);
    }
    // Try to parse file
    ...
    if (configExtends) {
      ...
      const extendsFile = resolve(dirname(file), configExtends);
...
function readConfigSync(file, parsers) {
  // Read file
  const content = fs.readFileSync(file, utf8Encoding);
  // Try to parse file
  const { config, message } = parseConfiguration(file, content, parsers);
  if (!config) {
  ...
  if (configExtends) {
    ...
    return {
      ...readConfigSync(
        resolve(dirname(file), configExtends),
        parsers
      ),
      ...config
    };
